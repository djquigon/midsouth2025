<?php
    //In this particular case, we're using a title-area.php file that is customized for the Galleries archive because the M&R Master Theme's design calls for the inclusion of a search & category form here.
?>
        ...
        <h1>
            <?php 
                if (isset($_GET['gallery-search']) || isset($_GET['gallery-category'])) {
                    ...
                    echo 'Filtered Galleries:';
                } elseif(is_tax('gallery_category')) {
                    //if we're looking at a specific resource category view:
                    echo get_the_archive_title();
                } else {
                    echo 'Media Galleries';
                }
            ?>
        </h1>
        <?php
            //if your galleries archive needs it:
            echo get_template_part('views/conditional/galleries/archive/widgets/search-category-form/search-category-form');
        ?>
        <span 
